...
router.post('/', auth, (req, res) => {
  const { name, world_name, world_description, character_description, appearance, personality } = req.body;
  ...
  const result = db.prepare(`
    INSERT INTO characters (user_id, name, world_name, world_description, character_description, appearance, personality)
    VALUES (?, ?, ?, ?, ?, ?, ?)
  `).run(req.user.id, name, world_name, world_description, character_description, appearance || '', personality || '');
...
router.put('/:id', auth, (req, res) => {
  ...
  const { name, world_name, world_description, character_description, appearance, personality } = req.body;
  db.prepare(`
    UPDATE characters SET name=?, world_name=?, world_description=?, character_description=?, appearance=?, personality=?
    WHERE id=?
  `).run(
    name || character.name,
